"""Hash-pinned process, environment and Shell profiles for the local Runtime."""
...
from .runtime_manifest import RuntimeFileRecord, RuntimeManifestError
...
class LocalProcessCatalogError(RuntimeError):
    """The local Runtime does not contain a safe canonical process catalog."""
...
def load_local_process_catalog(
    runtime_root: Path,
    *,
    manifest_trust: InstalledDevelopmentRuntimeTrust,
) -> LocalProcessCatalog:
    """Load the bounded catalog with explicit fixed-hash local trust."""
    ...
    except LocalProcessCatalogError:
        raise
    except (KeyError, OSError, TypeError, ValueError, RuntimeManifestError) as error:
        raise LocalProcessCatalogError(
            "local_process_catalog_invalid",
            "hash-pinned local process catalog is invalid",
        ) from error


def validate_process_catalog_payload(payload: bytes) -> str:
    """Validate one local catalog before it is copied into the Runtime."""
...
def _validated_catalog_specs(
    ...
    shell_specs = _shell_specs(raw["shellProfiles"])
    environment_by_id = {item.profile_id: item for item in environments}
...
def _parse_canonical_document(payload: bytes) -> Mapping[str, Any]:
    if not payload or len(payload) > _MAX_CATALOG_BYTES:
        raise LocalProcessCatalogError("process_catalog_size", "process catalog size is invalid")
    try:
        ...
        raise LocalProcessCatalogError("process_catalog_malformed", "process catalog is malformed") from error
    document = _mapping(value, "process catalog")
    _exact_keys(
        document,
        {"environmentProfiles", "executableProfiles", "schemaVersion", "shellProfiles"},
        "process catalog",
    )
    if _integer(document["schemaVersion"], "schemaVersion", minimum=1, maximum=1) != 1:
        raise LocalProcessCatalogError("process_catalog_schema", "process catalog schema is unsupported")
...
def _executable_specs(value: object) -> tuple[_ExecutableSpec, ...]:
    ...
    for item in values:
        ...
        if _text(raw["trust"], "trust", maximum=32) != ExecutableTrust.FIXED_HASH.value:
            raise LocalProcessCatalogError(
        ...
        if _boolean(raw["allowShellMetacharacters"], "allowShellMetacharacters"):
            raise LocalProcessCatalogError(
                ...
                "local process catalog cannot authorize shell metacharacters",
            )
...
def _shell_specs(value: object) -> tuple[_ShellSpec, ...]:
    values = _bounded_sequence(value, "shellProfiles", maximum=_MAX_SHELL_PROFILES, minimum=1)
    ...
    for item in values:
        raw = _mapping(item, "Shell profile")
        _exact_keys(raw, expected, "Shell profile")
        if _boolean(raw["allowNetwork"], "allowNetwork"):
            raise LocalProcessCatalogError(
                "process_catalog_network",
                "local Shell profiles cannot authorize network access",
            )
        minimum = _integer(raw["minimumVariableArguments"], "minimumVariableArguments", minimum=0, maximum=128)
        maximum = _integer(raw["maximumVariableArguments"], "maximumVariableArguments", minimum=0, maximum=128)
        if minimum > maximum:
            raise LocalProcessCatalogError("process_catalog_arguments", "Shell argv bounds are inverted")
        ...
        try:
            ...
            raise LocalProcessCatalogError(
                ...
                "Shell profile risk is invalid",
            ) from error
        version = _text(raw["version"], "version", maximum=64)
        if _VERSION.fullmatch(version) is None:
            raise LocalProcessCatalogError("process_catalog_version", "Shell profile version is invalid")
        specs.append(
            _ShellSpec(
    ...
    _require_sorted_unique((item.profile_id for item in specs), "shellProfiles")
    return tuple(specs)
...
def _validate_shell_spec_binding(
    ...
    if executable is None or environment_profile is None:
        raise LocalProcessCatalogError(
            ...
            "Shell profile references an unknown process profile",
        )
    if spec.cwd_root_id not in executable.allowed_cwd_root_ids:
        raise LocalProcessCatalogError(
            "process_catalog_shell_binding",
            "Shell profile cwd exceeds its executable profile",
        )
    if spec.environment_profile_id not in executable.environment_profile_ids:
        raise LocalProcessCatalogError(
            "process_catalog_shell_binding",
            "Shell environment exceeds its executable profile",
        )
    if not spec.environment_allowlist <= environment_profile.allowed_names:
        raise LocalProcessCatalogError(
            "process_catalog_shell_binding",
            "Shell environment allowlist exceeds its environment profile",
        )
    prefix = executable.fixed_arguments
    if spec.fixed_arguments[: len(prefix)] != prefix:
        raise LocalProcessCatalogError(
            "process_catalog_shell_binding",
            "Shell argv does not retain its executable prefix",
        )
    ...
    if (
        ...
        raise LocalProcessCatalogError(
            "process_catalog_shell_binding",
            "Shell argv exceeds its executable profile",
        )
...
def _read_manifest_asset(root: Path, record: RuntimeFileRecord, *, maximum: int) -> bytes:
    if record.byte_length < 1 or record.byte_length > maximum:
        raise LocalProcessCatalogError("process_catalog_size", "process catalog manifest size is invalid")
    ...
    if (
        ...
        raise LocalProcessCatalogError(
            ...
            "process catalog asset differs from its hash-pinned manifest record",
        )
...
def _manifest_path(root: Path, record: RuntimeFileRecord) -> Path:
    ...
    try:
        ...
        raise LocalProcessCatalogError(
            ...
            "process catalog path escaped the local Runtime",
        ) from error
    return path
...
def _mapping(value: object, label: str) -> Mapping[str, Any]:
    if not isinstance(value, dict) or any(not isinstance(key, str) for key in value):
        raise LocalProcessCatalogError("process_catalog_type", f"{label} must be an object")
    return value


def _exact_keys(value: Mapping[str, Any], expected: set[str], label: str) -> None:
    if set(value) != expected:
        raise LocalProcessCatalogError("process_catalog_fields", f"{label} fields are not exact")
...
def _bounded_sequence(
    ...
    *,
    ...
    if not isinstance(value, list) or not minimum <= len(value) <= maximum:
        raise LocalProcessCatalogError("process_catalog_type", f"{label} must be a bounded array")
    return value
...
def _text(value: object, label: str, *, maximum: int, minimum: int = 0) -> str:
    if not isinstance(value, str) or not minimum <= len(value) <= maximum or "\x00" in value:
        raise LocalProcessCatalogError("process_catalog_type", f"{label} must be bounded text")
    return value


def _boolean(value: object, label: str) -> bool:
    if type(value) is not bool:
        raise LocalProcessCatalogError("process_catalog_type", f"{label} must be boolean")
    return value


def _integer(value: object, label: str, *, minimum: int, maximum: int) -> int:
    if type(value) is not int or not minimum <= value <= maximum:
        raise LocalProcessCatalogError("process_catalog_type", f"{label} must be a bounded integer")
    return value
...
def _profile_id(value: object, label: str) -> str:
    ...
    if _PROFILE_ID.fullmatch(text) is None:
        raise LocalProcessCatalogError("process_catalog_identity", f"{label} is invalid")
    return text
...
def _shell_profile_id(value: object) -> str:
    ...
    if _SHELL_PROFILE_ID.fullmatch(text) is None:
        raise LocalProcessCatalogError("process_catalog_identity", "Shell profileId is invalid")
    return text
...
def _environment_names(value: object, label: str) -> tuple[str, ...]:
    ...
    if any(_ENVIRONMENT_NAME.fullmatch(item) is None for item in values):
        raise LocalProcessCatalogError("process_catalog_environment", f"{label} contains an invalid name")
...
def _text_mapping(value: object, label: str, *, maximum: int) -> dict[str, str]:
    ...
    if len(raw) > maximum:
        raise LocalProcessCatalogError("process_catalog_type", f"{label} is too large")
    result: dict[str, str] = {}
    for key, item in raw.items():
        if _ENVIRONMENT_NAME.fullmatch(key) is None:
            raise LocalProcessCatalogError("process_catalog_environment", f"{label} key is invalid")
...
def _pattern(value: object, label: str) -> str:
    ...
    try:
        ...
        raise LocalProcessCatalogError("process_catalog_pattern", f"{label} is invalid") from error
    return pattern
