"""Canonical JSON encoding and SHA-256 identities without runtime authority."""
...
def canonical_json_bytes(value: Any) -> bytes:
    """Encode the RFC 8785/JCS subset shared by every local identity boundary.

    Objects are ordered by UTF-16 code units and numbers use ECMAScript's
    fixed/scientific thresholds. Integers outside JavaScript's exact range fail
    closed and must be represented as strings by the calling schema. This keeps
    Python Runtime and generated TypeScript clients on one hash contract.
    """

    try:
        text = _encode_jcs(value, seen=set())
