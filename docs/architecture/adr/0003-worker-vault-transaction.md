...
短暂 UI 渲染故障或事件重放不会回滚已经提交的 Worker 事实，也不会切换文件执行路径。插件实例的
生命周期结束则会停止它直接创建的 Worker；活动 Run 必须按取消/中断和既有恢复协议明确收口，不能
转交后台进程继续执行。工具是否存在、是否对当前 Run 可见、是否获准执行以及实际结果分别记录，
UI 只渲染这些权威事件。
